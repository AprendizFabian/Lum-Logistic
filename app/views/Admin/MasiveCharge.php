  ...
  <div class="bg-white rounded-2xl shadow-xl p-6 border-t-4 border-[#FEE000]">
    ...
    <form id="formCargueMasivo" enctype="multipart/form-data">
      <?php if ($_SESSION['user']['type'] === 'user'): ?>
        <div class="mb-4">
          <label class="block text-gray-700 font-semibold mb-4">Selecciona la tienda en la que estas antes de subir el archivo</label>
...
<script>
  ...
  // -------- Selector de archivo con alerta plantilla --------
  archivoInput.addEventListener('click', async function handler(e) {
    e.preventDefault(); // prevenir apertura inmediata
    ...
    if (resultado.isConfirmed) {
      ...
      return; // salir para no abrir el selector
    }

    if (resultado.dismiss === Swal.DismissReason.cancel) {
      archivoInput.removeEventListener('click', handler);
      archivoInput.click(); // abrir selector
      setTimeout(() => archivoInput.addEventListener('click', handler), 50); // reactivar listener
    }
  });
...
formCargueMasivo.addEventListener("submit", async e => {
  ...
  if (!archivoInput.files.length) {
    archivoInput.click(); // abrir selector si no hay archivo
    return;
  }
  ...
  try {
    ...
    // ✅ Limpiar input y display siempre
    archivoInput.value = "";
    fileNameDisplay.textContent = "";
    ...
    // ✅ Limpiar input y display en caso de error
    archivoInput.value = "";
    fileNameDisplay.textContent = "";
  }
});
